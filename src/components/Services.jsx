    ...
    <section className="my-12 xl:my-24 container mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {carsInfoList.map((item, index) => {
          return (
            <div
              className="bg-white rounded-xl px-4 xl:py-4 py-4 grid gap-4 hover:cursor-pointer hover:shadow-md"
              key={index}
            >
              ...
              <div className="flex flex-col gap-1 items-start">
                ...
                <h3 className="text-xl text-headingPrimary font-semibold my-3 capitalize">
                  {item.name}
                </h3>
                <div className="flex items-center gap-1">
                  <span className="text-orangePrimary text-2xl font-bold">
                    {item.price}
                  </span>
                  <span className="text-gray-600 font-medium">
                    {item.monthlyPrice}{" "}
                    <span className="text-textPrimary">/monthly</span>
                  </span>
                </div>
                <Button
                  btnText={`rent now`}
                  btnStyle={`bg-orangePrimary w-full my-3 text-white capitalize font-medium`}
                />
              </div>
            </div>
          );

      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 xl:gap-8 gap-4">
        {
          reviewList.map((item, index) => (
            <div key={index} className="bg-white rounded-lg shadow-md hover:shadow-lg group-hover:transition duration-200 group-hover:-translate-y-2 p-8 flex flex-col gap-4 items-center">
              ...
              <h3 className="text-xl capitalize text-headingPrimary font-semibold">{item.name}</h3>
              <div className="flex items-center gap-2 text-base font-light">
                <span className={`text-orangePrimary font-semibold`}>{item.rating}</span>
                <span className='text-orangePrimary'><FaStar />
</span>
              </div>
